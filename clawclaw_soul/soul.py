...
def generate_badge(
    ...
    Returns dict with:
    - 'markdown': shields.io badge markdown
    - 'svg_url': direct SVG URL
    - 'html': HTML img tag
    - 'snippet': full README snippet (badge + link to SOUL.md)
    - 'lagna': the lagna sign
    - 'archetype': persona archetype
    """
    card = soul.card
    lagna = card["lagna"]
    archetype = card["persona"].get("archetype", lagna)
    color = "E8A838"  # Wise Gold from brand palette

    label = "Soul"
    message = f"{lagna}%20%C2%B7%20Verified"
    svg_url = f"https://img.shields.io/badge/{label}-{message}-{color}?style={style}"

    markdown = f"[![ClawClaw Soul]({svg_url})](https://github.com/awrshift/clawclaw-soul)"
    html = f'<a href="https://github.com/awrshift/clawclaw-soul"><img src="{svg_url}" alt="ClawClaw Soul"></a>'

    snippet = f"""{markdown}

<!-- Generated by ClawClaw Soul v{_VERSION} -->
<!-- Agent: {agent_name} | Lagna: {lagna} | Archetype: {archetype} -->
<!-- Verify: clawclaw-soul verify SOUL.md -->"""
